...
def compute_layer_norms(states: List[List[torch.Tensor]]) -> Dict[str, float]:
    """Compute mean absolute value (Norm) for each layer's states."""
    norms = {}
    for layer_idx, state_list in enumerate(states):
        for state_idx, state in enumerate(state_list):
            key = f"Norm/layer_{layer_idx}_state_{state_idx}"
            norms[key] = torch.abs(state).mean().item()
    return norms


def compute_layer_saturations(states: List[List[torch.Tensor]]) -> Dict[str, float]:
    """Compute saturation (fraction at 0) for each layer's states."""
    saturations = {}
    for layer_idx, state_list in enumerate(states):
        for state_idx, state in enumerate(state_list):
            key = f"Saturation/layer_{layer_idx}_state_{state_idx}"
...
def compute_top5_error(logits: torch.Tensor, targets: torch.Tensor) -> float:
    """Compute top-5 error rate."""
    _, top5_pred = logits.topk(5, dim=1)
    correct = top5_pred.eq(targets.view(-1, 1).expand_as(top5_pred)).any(dim=1)
    return (1.0 - correct.float().mean().item()) * 100  # percentage
...
def evaluate_with_stats(model, dataloader, device: str, n_iters_infer: int = 120, streams=None):
    ...
    for x, y in tqdm(dataloader, desc="Evaluating"):
        ...
        # Layer norms and saturations
        batch_norms = compute_layer_norms(free_states)
        batch_saturations = compute_layer_saturations(free_states)
...
def _train_loop():
    ...
    parser.add_argument('--wandb_project', type=str, default='EqProp-New', help="Wandb project name")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Accumulators for layer stats (training)
        ...
        for step, (x, y) in enumerate(iterator):
            ...
            # Accumulate layer norms and saturations from free_states
            if wandb.run is not None:
                batch_norms = compute_layer_norms(previous_states)
                batch_saturations = compute_layer_saturations(previous_states)
                for k, v in batch_norms.items():
        ...
        if wandb.run is not None:
            ...
            # Average layer norms and saturations for training
