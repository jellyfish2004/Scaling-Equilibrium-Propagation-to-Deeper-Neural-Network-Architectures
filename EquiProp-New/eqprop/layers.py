...
class Interaction(nn.Module):
    ...
    def backward(self, pre, post):
        """Compute both grad_post and grad_pre. Returns (grad_post, grad_pre)."""
        raise NotImplementedError

    def backward_post(self, pre, post):
        """Compute only grad_post (gradient of energy w.r.t. post state)."""
        raise NotImplementedError

    def backward_pre(self, pre, post):
        """Compute only grad_pre (gradient of energy w.r.t. pre state)."""
        raise NotImplementedError
...
class InteractionConv2d(Interaction):
    ...
    def backward_post(self, pre, post):
        """Compute only grad_post: forward conv + bias. Skips conv_transpose2d."""
        with torch.no_grad():
            feat = self.conv(pre)
            bias = self.bias.view(1, -1, 1, 1)
            return -(feat + bias)
    ...
    def backward_pre(self, pre, post):
        """Compute only grad_pre: conv_transpose2d. Skips forward conv."""
        with torch.no_grad():
            output_padding = (self._stride_tuple[0] - 1, self._stride_tuple[1] - 1)
...
class InteractionConvMaxPool2d(Interaction):
    ...
    def backward_post(self, pre, post):
        """Compute only grad_post: forward conv + pool + bias. Skips unpool + conv_transpose2d."""
        with torch.no_grad():
            feat = self.conv(pre)
            feat_pooled = F.max_pool2d(feat, 2)
            bias = self.bias.view(1, -1, 1, 1)
            return -(feat_pooled + bias)

    def backward_pre(self, pre, post):
        """Compute only grad_pre: forward conv (for indices) + unpool + conv_transpose2d."""
        with torch.no_grad():
            feat = self.conv(pre)
            feat_pooled, indices = F.max_pool2d(feat, 2, return_indices=True)
            post_unpool = F.max_unpool2d(post, indices, kernel_size=2, stride=2, output_size=feat.shape)
...
class InteractionLinear(Interaction):
    ...
    def backward_post(self, pre, post):
        """Compute only grad_post: linear(pre) + bias. Skips matmul(post, W)."""
        with torch.no_grad():
            if pre.dim() > 2:
                pre_flat = pre.reshape(pre.size(0), -1).contiguous()
            else:
                pre_flat = pre
            grad_post = -self.linear(pre_flat)
            return grad_post - self.bias.view(1, -1)

    def backward_pre(self, pre, post):
        """Compute only grad_pre: matmul(post, W). Skips linear(pre)."""
        with torch.no_grad():
            grad_pre_flat = -torch.matmul(post, self.linear.weight)
            return grad_pre_flat.reshape(pre.shape)
