...
def main():
    ...
    # VGG5 / 5-layer conv model - smaller than ResNet13
    # weight_gains from original VGG5 defaults, see dhcn.py or similar training scripts
    ...
    # === Network wrapper (from model/function/network.py: Network) ===
    network = Network(energy_fn)

    # === Cost function (from model/function/cost.py: SquaredError) ===
    output_layer = energy_fn.layers()[-1]
    cost_fn = SquaredError(output_layer)

    # === Augmented function (from training/sgd.py: AugmentedFunction) ===
    augmented_fn = AugmentedFunction(energy_fn, cost_fn)

    # === Minimizers (from model/hopfield/minimizer.py: FixedPointMinimizer) ===
    params = energy_fn.params()
    layers = energy_fn.layers()
    free_layers = network.free_layers()

    # Inference minimizer (free phase)
    ...
    # Training minimizer (nudged phase)
    ...
    # === Gradient estimator (from training/sgd.py: EquilibriumProp) ===
    estimator = EquilibriumProp(params, layers, augmented_fn, cost_fn, minimizer_training)
    estimator.variant = 'centered'  # See training/sgd.py lines ~85-100
    estimator.nudging = args.beta

    # === Optimizer (from training/monitor.py: Optimizer) ===
    # VGG5 has 5 biases + 5 weights = 10 params
    ...
    # === Training loop (from training/epoch.py: Trainer.run) ===
    global_batch_idx = 0
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(training_loader):
            ...
            # === Free Phase (from training/epoch.py: Trainer.run, lines ~195-200) ===
            # Corresponds to: network.set_input(x); minimizer_inference.compute_equilibrium()
            network.set_input(x, reset=False)
            minimizer_inference.compute_equilibrium()
            
            # Store free states (layers at equilibrium with nudging=0)
            free_states = [layer.state.detach().clone() for layer in layers]
            
            if should_dump:
                save_states(layers, f'{dump_prefix}_free_states.pt')
                
                # Compute free energy (from model/function/function.py: SumSeparableFunction.eval)
                with torch.no_grad():
                    energy_free = energy_fn.eval().mean().item()

            # === Set target (from model/function/cost.py: SquaredError.set_target) ===
            cost_fn.set_target(y)

            # === Positive Nudged Phase (from training/sgd.py: EquilibriumProp.compute_gradient) ===
            # See lines ~120-130: first phase with _first_nudging = +beta
            # Restore free states
            for layer, state in zip(layers, free_states):
                layer.state = state.clone()
            
            augmented_fn.nudging = args.beta  # training/sgd.py: AugmentedFunction.nudging
            ...
            # === Negative Nudged Phase (from training/sgd.py: EquilibriumProp.compute_gradient) ===
            # See lines ~130-140: second phase with _second_nudging = -beta
            # Restore free states again
            for layer, state in zip(layers, free_states):
                layer.state = state.clone()
            ...
            # === Compute Gradients (from training/sgd.py: EquilibriumProp.compute_gradient) ===
            # See lines ~140-170: _standard_param_grads or _alternative_param_grads + clipping
            # Restore free states for gradient computation
            for layer, state in zip(layers, free_states):
                layer.state = state.clone()
            ...
            # === Update Parameters (from training/epoch.py: Trainer.run lines ~210-220) ===
            # Corresponds to: param.state.grad = grad; optimizer.step(); param.clamp_()
            all_params = params + cost_fn.params()
            for param, grad in zip(all_params, grads):
                param.state.grad = grad
            
            # From training/monitor.py: Optimizer.step
            optimizer.step()
            
            # Weight clamping (from model/function/param.py: Param.clamp_)
